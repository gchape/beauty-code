...
export const QuantityStepper = ({ item }: QuantityStepperProps) => {
  const dispatch = useCartDispatch();

  return (
    <div className="inline-flex items-center rounded-full bg-pink-100 border border-pink-200">
      <button
        onClick={() => dispatch({ item, action: "DECREASE" })}
        disabled={item.quantity === 1}
        aria-label="რაოდენობის შემცირება"
        className="flex items-center justify-center w-7 h-7 rounded-full text-taupe-600
      ...
      <span
        className="flex items-center justify-center w-7 text-center
